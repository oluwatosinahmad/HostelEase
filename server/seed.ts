...
export function runSeed() {
  ...
  db.transaction(() => {
    ...
    // 4c. Seed Verified and Pending Accommodation Agents (4th Role)
    const agentPasswordHash = bcrypt.hashSync('Agent123!', 10);
    const agent1Id = 'user-agent-1';
    const agent2Id = 'user-agent-2';

    // Agent 1: Approved & Verified
    insertUser.run(
      agent1Id,
      'agent1@hostelease.ng',
      agentPasswordHash,
      'Bamidele Olatunji',
      '+2348039876543',
      'AGENT',
    ...
    db.prepare(`
      INSERT INTO agent_profiles (
        id, user_id, business_name, operating_areas_json, experience_years, bio, verification_status,
        id_document_url, id_document_type, service_fee_amount, rating, review_count, completed_requests_count,
        active_students_count, payout_bank_name, payout_account_number, payout_account_name, verified_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `).run(
      'ap-1',
      agent1Id,
      'Bamidele Student Relocations',
      JSON.stringify(['Under G', 'Adenike', 'Stadium Road']),
      4,
      'Certified student lodge specialist with 4+ years assisting LAUTECH freshers and staylites secure vetted accommodations.',
      'APPROVED',
      '/uploads/verification_documents/nin_agent_bamidele.pdf',
      'NIN_CARD',
      5000,
      4.9,
      18,
    ...
    db.prepare(`
      INSERT INTO agent_profiles (
        id, user_id, business_name, operating_areas_json, experience_years, bio, verification_status,
        id_document_url, id_document_type, service_fee_amount, rating, review_count, completed_requests_count, active_students_count
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      'ap-2',
      agent2Id,
      'Ogbomoso Campus Connect',
      JSON.stringify(['College Road', 'General Area']),
      2,
      'Helping medical and nursing students locate quiet lodges close to LAUTECH Teaching Hospital.',
      'PENDING',
      '/uploads/verification_documents/voters_card_sadiq.pdf',
      'VOTERS_CARD',
      5000,
      5.0,
      0,
    ...
    // Seed Sample Agent Requests
    db.prepare(`
      INSERT INTO agent_requests (
        id, student_id, agent_id, property_id, preferred_areas_json, budget_min, budget_max, room_type, move_in_date, status, notes, service_fee, fee_payment_status
      ) VALUES (
        'req-seed-1', ?, ?, NULL, '["Under G", "Adenike"]', 200000, 300000, 'SELF_CONTAIN', '2026-09-15', 'IN_PROGRESS', 'Need clean ensuite self-contain with steady water and prepaid meter.', 5000, 'ESCROW'
      )
    `).run(studentId, agent1Id);

    db.prepare(`
      INSERT INTO agent_requests (
        id, student_id, agent_id, property_id, preferred_areas_json, budget_min, budget_max, room_type, move_in_date, status, notes, service_fee, fee_payment_status
      ) VALUES (
        'req-seed-2', ?, NULL, NULL, '["Stadium Road", "College Road"]', 180000, 250000, 'SINGLE_ROOM', '2026-10-01', 'OPEN', 'Looking for a serene room near LAUTECH 2nd gate for 400L final year project.', 5000, 'UNPAID'
      )
    `).run(studentId);

    // Seed Agent Earnings
    db.prepare(`
      INSERT INTO agent_earnings (id, agent_id, amount, earning_type, status, notes)
      VALUES ('ern-seed-1', ?, 5000, 'SERVICE_FEE', 'AVAILABLE', 'Completed hostel placement for Bolanle Olamide at Under G Royal Suites')
    `).run(agent1Id);
    db.prepare(`
      INSERT INTO agent_earnings (id, agent_id, amount, earning_type, status, notes)
      VALUES ('ern-seed-2', ?, 12500, 'COMMISSION', 'AVAILABLE', 'Landlord representation commission for Adeleke Villa')
    `).run(agent1Id);

    // Seed Agent Lead
    db.prepare(`
      INSERT INTO agent_leads (id, agent_id, hostel_name, area_id, landmark, estimated_rent, room_types, landlord_name, landlord_phone, notes, status)
      VALUES ('lead-seed-1', ?, 'Harmony Student Villa', 'area-under-g', 'Opposite Bovas Petrol Station', 220000, 'Self-Contain & Single Rooms', 'Pa Adeyemi', '08023456789', 'Brand new 16-room hostel with perimeter fencing and clean borehole.', 'PENDING_VERIFICATION')
    `).run(agent1Id);
