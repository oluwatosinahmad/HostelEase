...
export const HostelCreationWizard: React.FC<HostelCreationWizardProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in-50 duration-200 text-slate-900 dark:text-slate-100">
      ...
      <div className="bg-slate-950 text-white p-4 sm:p-6 pb-4 border-b border-slate-800">
        <div className="flex items-center justify-between flex-wrap gap-2 sm:gap-3 mb-3 sm:mb-4">
          <div>
            ...
            <h2 className="text-base sm:text-xl font-black mt-0.5">
              {initialData ? 'Edit Hostel Listing' : 'List New Student Accommodation'}
            </h2>
          </div>

          {/* Completeness Indicator */}
          <div className="flex items-center gap-2 bg-white/10 px-2.5 py-1 sm:px-3 sm:py-1.5 rounded-xl text-[11px] sm:text-xs">
            <span className="text-slate-300 hidden xs:inline">Completeness:</span>
            <div className="w-16 sm:w-20 bg-slate-700 h-2 rounded-full overflow-hidden">
              <div 
        ...
        {/* Mobile Step Header & Progress (< sm) */}
        <div className="sm:hidden pt-3 border-t border-slate-800 space-y-2">
          <div className="flex items-center justify-between text-xs">
            <span className="font-bold text-emerald-400 flex items-center gap-1.5">
              ...
              <span className="text-white font-bold">{steps[currentStep - 1]?.label}</span>
              <span className="text-slate-400 text-[10px]">({currentStep} of 8)</span>
            </span>
            <div className="flex items-center gap-1">
              {currentStep > 1 && (
                <button
                  ...
                  className="px-2 py-1 bg-white/10 hover:bg-white/20 text-[10px] font-bold rounded-lg text-slate-200 transition-colors"
                >
                  ‹ Prev
                </button>
              )}
              {currentStep < 8 && (
                <button
                  type="button"
                  onClick={() => setCurrentStep(prev => prev + 1)}
                  className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-[10px] font-bold rounded-lg text-white transition-colors"
                >
                  Next ›
                </button>
              )}
          ...
          {/* Quick Step Tap Pills */}
          <div className="flex items-center gap-1 overflow-x-auto scrollbar-none py-1 -mx-1 px-1">
            {steps.map(step => (
              <button
                key={step.num}
                type="button"
        ...
        {/* Desktop Step Indicator Bar (sm+) */}
        <div className="hidden sm:grid sm:grid-cols-8 gap-1 pt-2 border-t border-slate-800 text-[10px]">
          {steps.map(step => (
            <button
              key={step.num}
              type="button"
          ...
          <div className="space-y-4 max-w-3xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button
                ...
                className="self-start sm:self-auto px-3.5 py-2 bg-emerald-100 dark:bg-emerald-900/60 text-emerald-800 dark:text-emerald-300 font-bold text-xs rounded-xl flex items-center gap-1.5 hover:bg-emerald-200 dark:hover:bg-emerald-800 transition-colors cursor-pointer shrink-0"
              >
            ...
            <div className="space-y-3">
              {roomsList.map((room, idx) => (
                <div key={idx} className="bg-slate-50 dark:bg-slate-800/80 p-3.5 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full text-xs font-semibold bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-xl px-3 py-2.5 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 focus:outline-none"
                      />
                    </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-4 text-xs font-semibold text-slate-700 dark:text-slate-300 pt-1">
      ...
      {/* Footer Controls */}
      <div className="bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <button
          type="button"
          onClick={onCancel}
          className="order-2 sm:order-1 px-4 py-2.5 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors text-center cursor-pointer"
        >
          Cancel
        </button>

        <div className="order-1 sm:order-2 flex items-center justify-end gap-2 flex-wrap sm:flex-nowrap">
          {currentStep > 1 && (
            <button
              type="button"
              onClick={() => setCurrentStep(prev => prev - 1)}
              className="px-3.5 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center gap-1 transition-colors cursor-pointer"
            >
          ...
          <button
            ...
            className="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold rounded-xl shadow flex items-center gap-1.5 disabled:opacity-50 transition-colors cursor-pointer"
          >
          ...
          {currentStep < 8 ? (
            <button
              type="button"
              onClick={() => setCurrentStep(prev => prev + 1)}
              className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow flex items-center gap-1 transition-colors cursor-pointer"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black rounded-xl shadow-lg flex items-center gap-1.5 disabled:opacity-50 transition-colors cursor-pointer"
            >
