...
export const SavedHostelsView: React.FC<SavedHostelsViewProps> = ({
  ...
  if (!isAuthenticated) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center space-y-4 text-slate-900 dark:text-slate-100">
        <div className="w-16 h-16 bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 rounded-3xl flex items-center justify-center mx-auto">
          <Bookmark className="w-8 h-8" />
        </div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Sign In to View Your Shortlist</h2>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-slate-900 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white flex items-center gap-2.5">
            <Bookmark className="w-7 h-7 text-emerald-600 dark:text-emerald-400 fill-emerald-600/20" />
        ...
        <button
          onClick={onNavigateToSearch}
          className="px-4 py-2 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 shadow-sm border border-transparent dark:border-slate-700"
        >
      ...
      {loading && (
        <div className="py-16 text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold">Loading your shortlisted lodges...</p>
        </div>
      )}
      ...
      {!loading && savedHostels.length === 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-12 text-center space-y-4 shadow-sm">
          ...
          <h3 className="font-bold text-base text-slate-800 dark:text-slate-100">Your Shortlist is Empty</h3>
          ...
          {savedHostels.map(property => {
            ...
            return (
              <div
                ...
                className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-emerald-300 dark:hover:border-emerald-600 transition-all overflow-hidden flex flex-col cursor-pointer group"
              >
                {/* Image & Badges */}
                <div className="relative aspect-[16/10] bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 right-3 flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="p-2 rounded-full bg-white/90 dark:bg-slate-900/90 text-rose-600 hover:bg-rose-100 dark:hover:bg-rose-950/60 shadow-md transition-all"
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-3">
                  <div className="space-y-1.5">
                    ...
                    {hasAvailabilityAlert && (
                      <div className="p-2 rounded-xl bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800 text-[10px] text-amber-800 dark:text-amber-300 font-bold flex items-center gap-1">
                  ...
                  <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                    <div>
                      <span className="text-[10px] text-slate-400 dark:text-slate-400 font-bold uppercase">Rent</span>
      ...
      {!loading && recentViews.length > 0 && (
        <div className="pt-6 border-t border-slate-200 dark:border-slate-800 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-base text-slate-900 dark:text-white">Recently Viewed Hostels</h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {recentViews.map(rv => (
              <div
                ...
                className="bg-white dark:bg-slate-900 p-3 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-emerald-300 dark:hover:border-emerald-600 shadow-sm cursor-pointer space-y-2 group transition-all"
              >
                <img
                  src={rv.coverImage}
                  alt={rv.title}
                  className="w-full h-24 rounded-xl object-cover bg-slate-100 dark:bg-slate-800 group-hover:scale-102 transition-transform"
                />
                <div>
                  <h4 className="font-bold text-xs text-slate-900 dark:text-white truncate group-hover:text-emerald-700 dark:group-hover:text-emerald-400 transition-colors">
                    {rv.title}
                  </h4>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">📍 {rv.areaName} • {formatDistance(rv.distanceFromCampusKm)}</p>
                  <p className="text-xs font-bold text-emerald-800 dark:text-emerald-400 mt-1">{formatNaira(rv.rentAmount)}/yr</p>
                </div>
              </div>
            ))}
          </div>
